...
<section class="price-section price-tariffs" id="tariffs"><div class="wrap"><div class="price-head" data-reveal><span class="sec-num">01</span><span class="eyebrow">Тарифы</span><h2>Выберите <em>формат охраны</em></h2><p>Семь основных вариантов для быстрого ориентира. Листайте карточки, сравнивайте режим, оснащение и подготовку сотрудников. Точная смета формируется после оценки объекта и задач.</p></div><div class="tariff-carousel" data-price-carousel><div class="tariff-viewport" data-carousel-viewport tabindex="0" aria-label="Карусель тарифов охранных услуг"><div class="tariff-track">{''.join(carousel_pairs)}</div></div><div class="tariff-controls"><div class="tariff-dots" aria-label="Навигация по тарифам">{carousel_dots}</div><div class="tariff-arrows"><button type="button" class="tariff-arrow" data-carousel-prev aria-label="Предыдущий тариф">‹</button><button type="button" class="tariff-arrow" data-carousel-next aria-label="Следующий тариф">›</button></div></div></div></div></section>
<section class="price-section alt" id="objects"><div class="wrap"><div class="price-head" data-reveal><span class="sec-num">02</span><span class="eyebrow">По типу объекта</span><h2>Найдите свой <em>объект</em></h2><p>Все варианты находятся в HTML страницы: фильтр нужен только для удобства пользователя и не скрывает каталог от поисковых и AI-роботов.</p></div><div class="price-tools"><input id="priceFilter" type="search" placeholder="Например: склад, магазин, ЖК, мероприятие" aria-label="Поиск по каталогу цен"></div><div class="price-table-wrap" data-reveal><table class="price-table"><thead><tr><th>Объект</th><th>Рекомендуемые услуги</th><th>Ориентир</th><th>Что влияет на цену</th><th>Страница</th></tr></thead><tbody>{''.join(object_rows)}</tbody></table></div><p class="price-note">{esc(PRICING['disclaimer'])}</p></div></section>
<section class="price-section alt"><div class="wrap"><div class="price-head" data-reveal><span class="sec-num">03</span><span class="eyebrow">Расчёт</span><h2>Что меняет <em>итоговую стоимость</em></h2><p>Ориентир в карточке помогает сравнить варианты, но договорная цена зависит от конкретной схемы охраны.</p></div><div class="factors"><div class="factor" data-reveal><b>График и состав</b><span>Режим поста, продолжительность смены и количество сотрудников.</span></div><div class="factor" data-reveal><b>Подготовка</b><span>Разряд, квалификация и требования к сотрудникам под задачу объекта.</span></div><div class="factor" data-reveal><b>Экипировка</b><span>Спецсредства, связь и вооружение там, где оно предусмотрено схемой охраны.</span></div><div class="factor" data-reveal><b>Технические средства</b><span>Сигнализация, видеонаблюдение, СКУД, тревожная кнопка и мониторинг.</span></div></div></div></section>
<section class="price-cta"><div class="wrap"><div class="price-cta-box" data-reveal><span class="sec-num price-cta-num">04</span><h2>Нужен точный расчёт?</h2><p>Сообщите тип объекта, адрес и режим работы. Подберём схему охраны и рассчитаем стоимость под конкретные задачи.</p><a class="btn btn-gold" href="/kontakty/">Рассчитать охрану</a></div></div></section>
</main>
{read_partial("footer.html")}
{read_partial("mobile-bar.html")}
{read_partial("chat.html")}
