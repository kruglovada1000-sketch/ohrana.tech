...
class Inspector(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if 'id' in attr and attr['id']: self.ids.append(attr['id'] or '')
        if tag == 'a' and attr.get('href'): self.links.append(attr['href'] or '')
        if tag == 'script' and (attr.get('type') or '').lower() == 'application/ld+json':
            self.in_jsonld = True; self.current_jsonld = []
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == 'script' and self.in_jsonld:
            ...
            self.current_jsonld = []; self.in_jsonld = False
...
def inspect(path: Path) -> None:
    ...
    parser = Inspector(); parser.feed(text); parser.close()
    errors: list[str] = []
    duplicate_ids = sorted({value for value in parser.ids if parser.ids.count(value) > 1})
    if duplicate_ids: errors.append(f'duplicate ids: {duplicate_ids}')
    for tag in sorted(BALANCED):
        opened = parser.count_open[tag]; closed = parser.count_close[tag]
        if opened != closed: errors.append(f'<{tag}> balance {opened}/{closed}')
    if not parser.jsonld_chunks: errors.append('no JSON-LD')
    organization_count = 0
    for index, raw in enumerate(parser.jsonld_chunks, 1):
        try:
            parsed = json.loads(raw); organization_count += organization_declarations(parsed)
    ...
    for href in parser.links:
        ...
        if target is not None and not target.exists(): missing_links.append(href)
    if missing_links: errors.append('missing internal links: ' + ', '.join(sorted(set(missing_links))))
    ...
    print(f'HTML integrity OK: {path.relative_to(ROOT)}; {len(parser.links)} links, {len(parser.ids)} ids, {len(parser.jsonld_chunks)} JSON-LD blocks, Organization declarations={organization_count}')


def main() -> None:
    pages = pages_to_validate()
    for path in pages: inspect(path)
